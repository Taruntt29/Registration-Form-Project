...
@@ -9,58 +23,59 @@ function postUserData(event01)
...
function showOutput(obj)
{
    ...
    {   
        ...
        let mainClass = document.getElementById('users')
        let childClass = document.createElement('li')
        childClass.textContent = `${userObj.Name}  -  ${userObj.Email}  -  ${userObj.Phone}  -  ${userObj.Time}`
        mainClass.append(childClass)
        let editBtn = document.createElement('button')
        editBtn.textContent = 'Edit'
        editBtn.style.backgroundColor = 'green'
        ...
        delBtn.addEventListener('click', ()=>
        {
            if(confirm('Delete User Info?'))
                {
                    mainClass.removeChild(childClass)
                    axios.delete("https://crudcrud.com/api/63249559f9554d179385aae28cdf8f06/userDetails")
